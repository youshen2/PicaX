...
private enum CoverColorSampler {
    nonisolated static func averageColor(url: URL?) async -> Color? {
        ...
        do {
            ...
            return boostedAccentColor(red: sample.red, green: sample.green, blue: sample.blue, count: sample.count)
            #else
            return nil
            #endif
        } catch {
    ...
    #if os(iOS)
    private nonisolated static func sampleComponents(data: Data) async -> (red: Int, green: Int, blue: Int, count: Int)? {
        await Task.detached(priority: .utility) {
            ...
            return (red: red, green: green, blue: blue, count: count)
        }.value
    }
    #endif

    private nonisolated static func normalizedColorComponent(_ value: Int, count: Int) -> Double {
        min(max(Double(value) / Double(count) / 255.0, 0), 1)
    }

    private nonisolated static func boostedAccentColor(red: Int, green: Int, blue: Int, count: Int) -> Color {
        let red = normalizedColorComponent(red, count: count)
        let green = normalizedColorComponent(green, count: count)
        let blue = normalizedColorComponent(blue, count: count)
        ...
        let boostedSaturation = min(max(saturation * 1.85, 0.48), 0.96)
        let boostedBrightness = min(max(maximum * 1.16, 0.68), 0.88)
